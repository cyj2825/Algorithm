...
# 가로 길이 : m, 세로 길이 : n, 쌓아올려지는 상자의 수 : h
...
# 해당 BFS 함수는 이미 이중 for문을 통해 1값을 갖는 x, y, z 좌표 값은 다 append 했으므로 안에서 그 작업을 해주지 않아도 됨
def BFS():
    # q가 빌 때까지 반복
    while q:
        # 앞서 이중 for문을 통해 1값을 갖는 x, y, z 좌표 값은 다 append 했으니까 그걸 꺼내서 익지 않은 토마토를 익히면 됨
        z, x, y = q.popleft()
        for i in range(6):
            nx = x + dx[i]
            ny = y + dy[i]
            nz = z + dz[i]
            # nx와 ny, nz가 좌표 안에 들어 있어야 하고 해당 좌표의 data값이 0이어야 함(토마토가 안 익은 상태)
